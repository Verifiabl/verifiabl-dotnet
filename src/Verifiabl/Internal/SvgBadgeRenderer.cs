...
/// </summary>
/// <remarks>
/// The QR matrix comes from the QR code generator library. Data modules are
/// rendered as standard square modules, with rounded finder patterns in the
/// three large corner sections. The output matches the Verifiabl Node.js SDK
/// renderer byte for byte for the same inputs.
/// </remarks>
internal static class SvgBadgeRenderer
{
    ...
    // badge's width.
    private const double MinModulePx = 3;
    ...
    // only levers are error-correction level and module size inside the fixed QR
    // box. Starting from the caller's ceiling (MaxErrorCorrection, default
    // Medium), we keep the highest correction level (best damage recovery:
    // Q ~25%, M ~15%, L ~7%) whose modules still clear the floor, stepping down
    // only when forced, and never varying the frame. A higher ceiling means more
    // modules (a denser code) for the same payload. Error correction is
    // invisible to the scan service, which only reads the decoded URL.
    private static readonly BarcodeErrorCorrectionLevel[] FullErrorCorrectionLadder =
    [
    ...
    internal static BarcodeSvgResult Render(BarcodeParts parts, BarcodeSvgOptions options)
    {
        double badgeWidth = ValidateBadgeWidth(options.Width);
        ...
        double height = Round2((badgeWidth * FrameViewboxHeight) / FrameViewboxWidth);
        double qrPadding = selected.InsetModules * selected.ModuleSize;

        string headerBackground =
            $"<path d=\"M0 8C0 3.58172 3.58172 0 8 0H88C92.4183 0 96 3.58172 96 8V" +
            $"{FrameHeaderHeight}H0V8Z\" fill=\"{DefaultNavy}\"/>";
        string header = headerBackground + RenderDefaultHeader(DefaultText);

        var svg = new StringBuilder();
        svg.Append("<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"")
            .Append(F(Round2(badgeWidth)))
            .Append("\" height=\"")
            .Append(F(Round2(height)))
            .Append("\" viewBox=\"0 0 ")
            .Append(FrameViewboxWidth)
            .Append(' ')
            ...
            .Append(' ')
            .Append(F(Round2(FrameQrBoxY + qrPadding)))
            .Append(")\">")
            .Append("<g shape-rendering=\"crispEdges\">")
            .Append(RenderModules(selected.Qr, selected.Size, selected.ModuleSize, DefaultQr))
            .Append("</g>")
            .Append(RenderFinders(selected.Size, selected.ModuleSize, DefaultQr))
            .Append("</g></svg>");

        return new BarcodeSvgResult(
            svg.ToString(),
            badgeWidth,
            Round2(height),
            content,
        ...
        BarcodeErrorCorrectionLevel ceiling)
    {
        ...
        int start = Array.IndexOf(FullErrorCorrectionLadder, ceiling);
        var ladder = new BarcodeErrorCorrectionLevel[FullErrorCorrectionLadder.Length - start];
        Array.Copy(FullErrorCorrectionLadder, start, ladder, 0, ladder.Length);
        return ladder;
    }
    ...
    /// ceiling) whose modules still clear <see cref="MinModulePx"/> at this width.
    /// The frame's outer width and height never change. Hard-errors if even the
        ...
        BarcodeErrorCorrectionLevel[] ladder)
    {
        ...
        foreach (BarcodeErrorCorrectionLevel level in ladder)
        {
            QrCode qr;
            try
            {
            ...
            {
                // The content exceeds capacity at this level; a lower level holds
                // more, so try it.
                continue;
            }
            ...
            {
                return new SelectedQrRendering(qr, level, size, moduleSize, modulePx, insetModules);
            }

            densestSize = size;
        }

        if (densestSize is null)
        {
            throw new InvalidOperationException(
                "The QR content (scan URL) is too large to encode in a QR code at any " +
                $"error-correction level ({content.Length} characters). Shorten the PII fields " +
                "and try again.");
        }
    ...
    {
        BarcodeErrorCorrectionLevel.Quartile => QrCode.Ecc.Quartile,
        BarcodeErrorCorrectionLevel.Medium => QrCode.Ecc.Medium,
        _ => QrCode.Ecc.Low,
    };
    ...
    private static int QuietZoneInsetModules(int size)
    {
        ...
        return QuietZoneModules;
    }

    private static string RenderModules(QrCode qr, int size, double moduleSize, string color)
    {
        ...
        for (int row = 0; row < size; row++)
        {
            for (int col = 0; col < size; col++)
            {
                ...
                double x = col * moduleSize;
                double y = row * moduleSize;
                parts.Append("<rect x=\"")
                    .Append(F(Round2(x)))
                    .Append("\" y=\"")
                    .Append(F(Round2(y)))
                    .Append("\" width=\"")
                    .Append(F(Round2(moduleSize)))
                    .Append("\" height=\"")
                    .Append(F(Round2(moduleSize)))
                    .Append("\" fill=\"")
                    .Append(color)
    ...
    {
        double lastFinderOrigin = (size - FinderSize) * moduleSize;
        return RenderFinder(0, 0, moduleSize, color)
            + RenderFinder(lastFinderOrigin, 0, moduleSize, color)
            + RenderFinder(0, lastFinderOrigin, moduleSize, color);
    }

    private static string RenderFinder(double originX, double originY, double moduleSize, string color)
    {
        double outer = FinderSize * moduleSize;
        double innerOffset = moduleSize;
        double inner = outer - moduleSize * 2;
        double dotOffset = moduleSize * 2;
        double dot = moduleSize * 3;
        string ringPath = RoundedRectPath(originX, originY, outer, outer, moduleSize * 1.4)
            + " "
            + RoundedRectPath(originX + innerOffset, originY + innerOffset, inner, inner, moduleSize);

        return $"<path d=\"{ringPath}\" fill=\"{color}\" fill-rule=\"evenodd\"/>"
            + $"<rect x=\"{F(Round2(originX + dotOffset))}\" y=\"{F(Round2(originY + dotOffset))}\" "
            + $"width=\"{F(Round2(dot))}\" height=\"{F(Round2(dot))}\" rx=\"{F(Round2(moduleSize * 0.65))}\" "
            + $"fill=\"{color}\"/>";
    }

    private static string RoundedRectPath(double x, double y, double width, double height, double radius)
    {
        double left = Round2(x);
        double top = Round2(y);
        double right = Round2(x + width);
        double bottom = Round2(y + height);
        double r = Round2(Math.Min(radius, Math.Min(width / 2, height / 2)));

        return $"M{F(Round2(x + radius))} {F(top)}H{F(Round2(x + width - radius))}"
            + $"A{F(r)} {F(r)} 0 0 1 {F(right)} {F(Round2(y + radius))}"
            + $"V{F(Round2(y + height - radius))}A{F(r)} {F(r)} 0 0 1 {F(Round2(x + width - radius))} {F(bottom)}"
            + $"H{F(Round2(x + radius))}A{F(r)} {F(r)} 0 0 1 {F(left)} {F(Round2(y + height - radius))}"
    ...
    private static double ValidateBadgeWidth(double value)
    {
        if (double.IsNaN(value) || double.IsInfinity(value) || value <= 0)
        {
            throw new ArgumentException(
                "Width must be a positive number.",
                nameof(BarcodeSvgOptions.Width));
        }

        if (value < MinBadgeWidth)
        {
            throw new ArgumentException(
                $"Width must be at least {MinBadgeWidth:0}.",
                nameof(BarcodeSvgOptions.Width));
        }
    ...
    internal readonly struct SelectedQrRendering
    {
            ...
            int insetModules)
        {
            ...
            ModulePx = modulePx;
            InsetModules = insetModules;
        }

        internal QrCode Qr { get; }
        ...
        internal int InsetModules { get; }
    }

    private const int WordmarkViewboxWidth = 80; // 80x16 design units

    private static string RenderWordmark(double x, double top, double renderWidth, string color)
    {
        double k = renderWidth / WordmarkViewboxWidth;
        return $"<g transform=\"translate({F(Round2(x))} {F(Round2(top))}) scale({F(Round2(k))})\" "
            + $"fill=\"{color}\">{WordmarkPaths}</g>";
    }

    private static string RenderDefaultHeader(string textColor)
    {
        return $"<g transform=\"translate(-8 0)\">{SecuredByPath.Replace("{color}", textColor)}</g>"
            + RenderWordmark(8, 23, 80, textColor);
    }

    /// <summary>
    /// Verifiabl wordmark as vector paths (official logo asset, 80x16 design
    /// units). Fills are applied by the containing group. The source clipPath is
    /// intentionally dropped: the paths stay in bounds and a fixed clipPath id
    /// would collide when multiple badges are inlined in one document.
    /// </summary>
    private const string WordmarkPaths =
    ...
    private const string SecuredByPath =
